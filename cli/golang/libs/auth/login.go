...
func requestJWT(ctx context.Context, config core.Config, device string) (*token.Token, error) {
	...
	resp, err := client.GetJwtToken(ctx, api.GetJwtTokenParams{
		ClientID:   api.NewOptString(config.ClientID),
		DeviceCode: device,
...
func Login(ctx context.Context) (*token.Token, error) {
	...
	case *api.DeviceFlow:
		...
		fmt.Println("Please verify this user code: ", r.UserCode)
		for {
			select {
			case <-ctx.Done():
				return nil, errors.New("could not authorize the CLI within a minute")
			default:
				time.Sleep(time.Duration(r.Interval+1) * time.Second)
				t, err := requestJWT(ctx, config, r.DeviceCode.String())
				if err != nil {
					return nil, err
				}
				if t == nil {
