...
Let people browse and install plugins written by others, with some form of review before something gets listed. This is not really a way of running code by itself — it's a way of finding it, and it could sit on top of any of the options above.
...
Not "this integration exists," but "this particular data product's use of it has these exact values filled in."
...
Take S3, used in three environments: dev, staging, production. Three tables hold facts about this today:
...
When someone creates an S3 asset, they fill in a form. The portal checks it against S3's fields. That's one filled-in, checked record, tied to one asset. Setting up "dev" for S3 is the same thing: fill in a form, check it against the same fields. Just tied to an environment instead of one asset, filled in by a platform engineer instead of a data product owner. Store both the same way, instead of building two different systems.
...
Delete all three tables. Replace them with one generic table: one record per plugin per environment, holding a checked blob of values — no separate vendor-level and service-level split, just one shape per plugin.
...
* **Good, because** it reuses the same self-describing mechanism already chosen for a plugin's metadata and its technical-asset values — one mechanism, three uses, instead of three separately designed systems.
