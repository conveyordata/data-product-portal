...
* A plugin can expose its own MCP tools by overriding `register_mcp_tools`, and add to the MCP server's instructions through its `mcp_instructions` class attribute, joined into the server's instructions only after every plugin has finished registering. Unlike request-time plugin calls, a plugin whose tools fail to register is not caught: MCP registration runs once at startup, the same as plugin discovery and plugin migrations, and a broken plugin there fails the portal's startup loudly rather than silently missing from the running server. Tools are built on the `fastmcp` version and the MCP dependencies (`app.mcp.deps`) portal core ships.
