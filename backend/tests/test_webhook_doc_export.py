...
def test_build_event_list_finds_all_events():
    events = build_event_list(app)
    assert len(events) == 27


def test_build_event_list_no_duplicates():
    events = build_event_list(app)
    keys = [(e["event"], e["method"], e["path"]) for e in events]
    assert len(keys) == len(set(keys))


def test_build_event_list_sorted_by_event_name():
    events = build_event_list(app)
    names = [e["event"] for e in events]
    assert names == sorted(names)


def test_known_data_product_event_present():
    events = build_event_list(app)
...
def test_known_output_port_event_present():
    events = build_event_list(app)
    event_types = {e["event"] for e in events}
...
def test_known_technical_asset_event_present():
    events = build_event_list(app)
    event_types = {e["event"] for e in events}
...
def test_data_product_created_event_shape():
    events = build_event_list(app)
...
def test_output_port_created_event_shape():
    events = build_event_list(app)
...
def test_technical_asset_created_event_shape():
    events = build_event_list(app)
...
def test_generate_markdown_contains_all_groups():
    events = build_event_list(app)
...
def test_generate_markdown_contains_event_rows():
    events = build_event_list(app)
    md = generate_markdown(events)
