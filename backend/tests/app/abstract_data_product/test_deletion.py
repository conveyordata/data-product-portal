...
class TestMarkForDeletion:
    def test_returns_true_when_no_finalizers(
        ...
        can_delete = AbstractDataProductService(db=session).mark_for_deletion(dp.id)
    ...
    def test_works_for_explorations_too(self, session):
        ...
        can_delete = AbstractDataProductService(db=session).mark_for_deletion(
            exploration.id
        )
    ...
    def test_raises_for_unknown_id(self, session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            AbstractDataProductService(db=session).mark_for_deletion(uuid.uuid4())
...
class TestAddFinalizer:
    def test_adds_finalizer(self, session):
        ...
        AbstractDataProductService(db=session).add_finalizer(dp.id, "my-system")
    ...
    def test_multiple_finalizers_accumulate(self, session):
        ...
        AbstractDataProductService(db=session).add_finalizer(dp.id, "system-a")
        AbstractDataProductService(db=session).add_finalizer(dp.id, "system-b")
    ...
    def test_duplicate_finalizer_is_idempotent(self, session):
        ...
        result = AbstractDataProductService(db=session).add_finalizer(dp.id, "existing")
    ...
    def test_add_finalizer_blocked_when_already_deleting(self, session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            AbstractDataProductService(db=session).add_finalizer(dp.id, "new-system")
...
class TestRemoveFinalizer:
    def test_remove_finalizer_not_deleting_returns_false(self, session):
        ...
        should_delete = AbstractDataProductService(db=session).remove_finalizer(
            dp.id, "my-system"
        )
    ...
    def test_remove_nonexistent_finalizer_raises_404(
        ...
        with pytest.raises(HTTPException) as exc_info:
            AbstractDataProductService(db=session).remove_finalizer(
                dp.id, "does-not-exist"
            )
...
class TestFullDeletionLifecycle:
    def test_add_finalizer_then_delete_then_remove_finalizer(self, session):
        ...
        AbstractDataProductService(db=session).add_finalizer(dp.id, "my-cleanup-job")
        can_delete = AbstractDataProductService(db=session).mark_for_deletion(dp.id)
...
class TestEnsureNotDeleting:
    def test_request_input_ports_blocked_for_deleting_consumer(self, session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            AbstractDataProductService(db=session).request_input_ports(
                id=consumer.id,
