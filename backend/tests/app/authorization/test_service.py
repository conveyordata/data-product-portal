...
class TestAuthorizationService:
    def test_reload_enforcer(self, authorizer: Authorization):
        existing = len(test_session.query(CasbinRule).all())
        ...
        assert len(test_session.query(CasbinRule).all()) == 5 + existing, (
            "roles not recorded"
        )

        service = AuthorizationService(test_session)
        service.reload_enforcer()

        assert len(test_session.query(CasbinRule).all()) == existing, (
