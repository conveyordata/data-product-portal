...
class Authorization(metaclass=Singleton):
    ...
    def remove_role_permissions(self, *, role_id: ID) -> None:
    ...
    def assign_global_role(self, *, user_id: ID, role_id: ID) -> None:
        ...
        enforcer: Enforcer = self._enforcer
        enforcer.add_named_grouping_policy("g3", str(user_id), str(role_id))
        self._after_update()

    def revoke_global_role(self, *, user_id: ID, role_id: ID) -> None:
        """Deletes the entry in the casbin table,
        revoking the chosen global role for the user."""
        enforcer: Enforcer = self._enforcer
        enforcer.remove_named_grouping_policy("g3", str(user_id), str(role_id))
        self._after_update()
    ...
    def clear_assignments_for_user(self, *, user_id: ID) -> None:
        ...
        enforcer: Enforcer = self._enforcer
        enforcer.remove_filtered_named_grouping_policy("g", 0, value)
        enforcer.remove_filtered_named_grouping_policy("g2", 0, value)
        enforcer.remove_filtered_named_grouping_policy("g3", 0, value)
        self._after_update()

    def clear_assignments_for_resource_role(self, *, role_id: ID) -> None:
        """Removes all assignments of a resource role inside the casbin table.
        Should be called when a resource role is removed.
        """
        enforcer: Enforcer = self._enforcer
        enforcer.remove_filtered_named_grouping_policy("g", 1, str(role_id))
        self._after_update()

    def clear_assignments_for_domain_role(self, *, role_id: ID) -> None:
        """Removes all assignments of a domain role inside the casbin table.
        Should be called when a domain role is removed.
        """
        enforcer: Enforcer = self._enforcer
        enforcer.remove_filtered_named_grouping_policy("g2", 1, str(role_id))
        self._after_update()

    def clear_assignments_for_global_role(self, *, role_id: ID) -> None:
        """Removes all assignments of a global role inside the casbin table.
        Should be called when a global role is removed.
        """
        enforcer: Enforcer = self._enforcer
        enforcer.remove_filtered_named_grouping_policy("g3", 1, str(role_id))
        self._after_update()

    def clear_assignments_for_resource(self, *, resource_id: ID) -> None:
        """Removes all assignments to a resource inside the casbin table.
        Should be called when a resource is removed.
        """
        enforcer: Enforcer = self._enforcer
        enforcer.remove_filtered_named_grouping_policy("g", 2, str(resource_id))
        self._after_update()

    def clear_assignments_for_domain(self, *, domain_id: ID) -> None:
        """Removes all assignments to a domain inside the casbin table.
        Should be called when a domain is removed.
        """
        enforcer: Enforcer = self._enforcer
        enforcer.remove_filtered_named_grouping_policy("g2", 2, str(domain_id))
        self._after_update()
