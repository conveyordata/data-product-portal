...
Welcome! These guidelines will help you navigate and contribute effectively to the Data Product Portal codebase.
Keep this context concise to minimize token usage while retaining critical architectural boundaries.

## Project Overview

The Data Product Portal manages data products and their output ports.
It consists of a multi-tier architecture with a FastAPI backend, a React frontend, a CLI, an MCP server, and comprehensive documentation.
...
*   **Code Quality**: Run `npm run biome` before committing. The pre-commit hook runs `npm run biome:fix` automatically.
