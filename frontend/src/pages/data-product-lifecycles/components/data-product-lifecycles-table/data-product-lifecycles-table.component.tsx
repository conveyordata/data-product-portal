...
import { useGetAllDataProductLifecyclesQuery, useUpdateDataProductLifecycleMutation } from '@/store/features/data-product-lifecycles/data-product-lifecycles-api-slice';
...
export function DataProductLifecyclesTable() {
    ...
    const components = {
      ...
      const editableColumns = columns.map((col: ColumnGroupType<DataProductLifeCycleContract>&{editable?: boolean}&{dataIndex: number} | ColumnType<DataProductLifeCycleContract>&{editable?: boolean}) => {
        if (!col.editable) {
          return col;
        }
        return {
