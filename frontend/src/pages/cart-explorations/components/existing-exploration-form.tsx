...
import { Alert, Button, Form, type FormProps, Select, Space, Typography } from 'antd';
...
export const ExistingExplorationForm = ({ cartOutputPorts, setSelectedExplorationId }: Props) => {
    ...
    const selectedExplorationId = Form.useWatch('selectedExplorationId', form);
    useEffect(() => {
    ...
    const { overlappingOutputPortIds, selectedProductOutputPortsInCartIds } =
        useCartOverlapCheck({selectedExplorationId});
    ...
    useEffect(() => {
        if (requestingAccessSuccess && selectedExplorationId) {
            ...
            navigate(createDataProductIdPath(selectedExplorationId, DataProductTabKeys.InputPorts));
        }
