...
import { RoleChangeForm } from '@/pages/dataset/components/dataset-tabs/team-tab/components/role-change-form/role-change-form.tsx';
import { DatasetMembershipRole, DatasetMembershipStatus, DatasetUserMembership } from '@/types/dataset-membership';
import { getDatasetMembershipBadgeStatus, getDatasetMembershipStatusLabel } from '@/utils/status.helper.ts';
...
type Props = {
    ...
    onRoleChange: (role: DatasetMembershipRole, membershipId: string) => void;
...
export const getDatasetUsersTableColumns = ({
    t,
    onRemoveMembership,
    ...
    canRemove,
    canApprove,
}: Props): TableColumnsType<DatasetUserMembership> => {
    const sorter = new Sorter<DatasetUserMembership>();
    return [
        {
            title: t('Id'),
            dataIndex: 'id',
        ...
        },
        {
            ...
            render: (_, { user, status }) => {
                const isNotApproved = status !== DatasetMembershipStatus.Approved;
                return (
                    <UserAvatar
        ...
        },
        {
            title: t('Role'),
            dataIndex: 'role',
            render: (role: DatasetMembershipRole, { user, id, status }) => {
                const isApproved = status === DatasetMembershipStatus.Approved;
                return (
                    <RoleChangeForm
                        initialRole={role}
                        userId={user.id}
            ...
            width: '25%',
            ...new FilterSettings(datasetUsers, (membership) => membership.role),
            sorter: sorter.stringSorter((membership) => membership.role),
        },
        {
            title: t('Status'),
            dataIndex: 'status',
            render: (status: DatasetMembershipStatus) => {
                return (
                    <Badge
                        status={getDatasetMembershipBadgeStatus(status)}
                        text={getDatasetMembershipStatusLabel(t, status)}
                    />
                );
            },
            width: '20%',
            ...new FilterSettings(datasetUsers, (membership) => getDatasetMembershipStatusLabel(t, membership.status)),
            sorter: sorter.stringSorter((membership) => getDatasetMembershipStatusLabel(t, membership.status)),
        },
        {
            title: t('Actions'),
            key: 'action',
            ...
            render: (_, { user, status, id }) => (
                <Space>
                    {status === DatasetMembershipStatus.Pending ? (
                        <Space>
                            <Popconfirm
                                title={t('Allow User')}
