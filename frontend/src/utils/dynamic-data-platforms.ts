...
export function generateDataPlatformsFromYaml(
    yamlContent: string,
    t: TFunction,
): CustomDropdownItemProps<DataPlatform>[] {
    try {
        const registry = yaml.load(yamlContent) as PlatformRegistry;

        // First, create a map of all platforms
        const platformMap = new Map<string, CustomDropdownItemProps<DataPlatform>>();
        ...
        for (const [key, config] of Object.entries(registry)) {
            const platformItem: CustomDropdownItemProps<DataPlatform> = {
                label: t(key),
                value: key.toLocaleLowerCase() as DataPlatform,
        ...
        const rootPlatforms: CustomDropdownItemProps<DataPlatform>[] = [];
 ...
 */
export function useDataPlatformsFromRegistry(yamlContent: string, t: TFunction) {
