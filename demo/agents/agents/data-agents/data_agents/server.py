...
data_dir = os.environ.get("AGENT_DATA_DIR", "/agent_data")
...
def make_admin_agent() -> Agent:
    return Agent(
        ...
        db=SqliteDb(db_file=f"{data_dir}/agno_generic_database_agent.db"),
...
def load_dynamic_agents():
    ...
    for filename in os.listdir(yaml_dir):
        ...
        agent = Agent(
            ...
            db=SqliteDb(
                db_file=f"{data_dir}/agno_{config['name'].replace(' ', '_')}.db"
            ),
            add_datetime_to_context=True,
