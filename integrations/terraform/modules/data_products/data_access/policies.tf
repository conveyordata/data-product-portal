...
resource "aws_iam_policy" "policy_read_generic" {
  name   = "${var.prefix}-${local.aws_iam}-${var.data_product_name}-read-generic-${var.environment}-${var.account_name}"
...
resource "aws_iam_policy" "policy_bucket_read" {
  ...
  name   = "${var.prefix}-${local.aws_iam}-${var.data_product_name}-bucket-read-${each.key}-${var.environment}-${var.account_name}"
...
resource "aws_iam_policy" "policy_write_generic" {
  name   = "${var.prefix}-${local.aws_iam}-${var.data_product_name}-write-generic-${var.environment}-${var.account_name}"
...
resource "aws_iam_policy" "policy_bucket_write" {
  ...
  name   = "${var.prefix}-${local.aws_iam}-${var.data_product_name}-bucket-write-${each.key}-${var.environment}-${var.account_name}"
  policy = each.value.json
}

resource "aws_iam_policy" "policy_glue_write" {
  name = "${var.prefix}-${local.aws_iam}-${var.data_product_name}-glue-write-${var.environment}-${var.account_name}"
