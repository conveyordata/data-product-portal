locals {
  ...
  data_product_glossary = {
    for k, v in local.data_product_glossary_raw : k => {
      ...
      users         = try([for user, user_role in v["users"] : user], [])
      business_unit = try(v["business_unit"], "")
      services = {
  ...
  data_outputs = {
    for k, v in local.data_outputs_raw : k => {
      ...
      glue = try([{
        database_identifier = v["glue"]["database"]
        database_suffix     = v["glue"]["database_suffix"]
        table               = v["glue"]["table"]
      ...
      dbx = try([{
        schema_identifier = v["databricks"]["schema_suffix"]
        bucket_identifier = v["databricks"]["bucket_identifier"]
        schema_path       = v["databricks"]["schema_path"]
        table             = try(v["databricks"]["table"], "*") # TODO: Remove this, it is backwards compatibility with implementation assuming table outputs
        table_path        = try(v["databricks"]["table_path"], "")
      }], [])
      owner = try(v["owner"], "")
    }
  }
  ...
  environments = {
    for environment, config in local.environments_raw : environment => {
      ...
      dbx_account_id         = config["dbx"]["account_id"]
      dbx_metastore_id       = config["dbx"]["metastore_id"]
      dbx_credential_name    = config["dbx"]["credential_name"]
      dbx_env_cluster_params = try(config["dbx"]["env_cluster_params"], {})
      business_units         = try(config["business_units"], [])
    }
  }
  # Create a map of environment-business_unit combinations
  workspace_combinations = merge([
    for env_name, env_config in local.environments : {
      for bu_name in env_config.business_units :
      "${bu_name}-${env_name}" => {
